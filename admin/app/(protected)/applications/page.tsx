...
function statusBadge(status: "pending" | "approved" | "rejected") {
  switch (status) {
    case "pending":
      return <Badge variant="outline" className="border-yellow-500/20 bg-yellow-500/10 text-yellow-500"><Clock className="mr-1 h-3 w-3" />Pending</Badge>;
    case "approved":
      return <Badge variant="outline" className="border-emerald-500/20 bg-emerald-500/10 text-emerald-500"><CheckCircle2 className="mr-1 h-3 w-3" />Approved</Badge>;
    case "rejected":
      return <Badge variant="outline" className="border-red-500/20 bg-red-500/10 text-red-400"><XCircle className="mr-1 h-3 w-3" />Rejected</Badge>;
  }
}

function getInitials(name: string | null, fallback: string) {
  return (name ?? fallback).split(/[\s@]/).filter(Boolean).map((w) => w[0]).join("").toUpperCase().slice(0, 2);
}
...
function ReviewButtons({ onApprove, onReject }: { onApprove: () => Promise<void>; onReject: () => Promise<void> }) {
  ...
  return (
    <div className="flex shrink-0 gap-2">
      <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700" disabled={loading !== null} onClick={() => handle("approve")}>
        {loading === "approve" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <CheckCircle2 className="h-3.5 w-3.5" />}
        Approve
      </Button>
      <Button size="sm" variant="destructive" disabled={loading !== null} onClick={() => handle("reject")}>
...
export default function ApplicationsPage() {
  ...
  const totalPending = pendingAdminApps.length + pendingProposals.length;

  return (
    <div className="space-y-6">
      <PageHeader title="Applications" description="Review admin applications and project proposals.">
              ...
              <Card key={app.id}>
                <CardContent className="pt-6">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    {app.status === "pending" && (
                      <ReviewButtons
                        onApprove={async () => { await clientApi.reviewAdminApplication(app.id, { status: "approved" }); load(); }}
                        onReject={async () => { await clientApi.reviewAdminApplication(app.id, { status: "rejected" }); load(); }}
                      />
                    )}
                  </div>
                </CardContent>
                ...
                <CardContent className="pt-6">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div className="flex-1">
                      ...
                      <div className="mt-3 flex items-center gap-2 text-xs text-muted-foreground">
                        <Avatar className="h-5 w-5">
                          <AvatarFallback className="text-[9px]">{getInitials(proposal.applicant.displayName, proposal.applicant.email)}</AvatarFallback>
                        </Avatar>
                    ...
                    {proposal.status === "pending" && (
                      <ReviewButtons
                        onApprove={async () => { await clientApi.reviewProjectProposal(proposal.id, { status: "approved" }); load(); }}
                        onReject={async () => { await clientApi.reviewProjectProposal(proposal.id, { status: "rejected" }); load(); }}
                      />
                    )}
                  </div>
                </CardContent>
