...
function roleBadgeClass(role: AdminPanelRole) {
  ...
  if (role === "admin") return "border-teal-400/40 bg-teal-400/10 text-teal-300";
...
export default function RolesPage() {
  const [matrix, setMatrix] = useState<RolesMatrixResponse | null>(null);
  ...
  useEffect(() => {
    ...
    clientApi
      .getRolesMatrix()
      .then((payload) => setMatrix(payload))
      .catch(() => setMatrix(buildLocalRolesMatrix()))
      .finally(() => setLoading(false));
  }, []);

  const roleOrder = useMemo<AdminPanelRole[]>(() => {
    if (!matrix) return ["owner", "admin", "moderator"];
    return matrix.governanceRoles.map((entry) => entry.role).filter(Boolean) as AdminPanelRole[];
  }, [matrix]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {(matrix?.governanceRoles ?? []).map((entry) => (
          <Card key={entry.role} className="border-primary/20 bg-gradient-to-br from-card to-card/70">
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Jump by role to inspect exactly what this role can access and mutate.
          </CardDescription>
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground">Unable to load role matrix.</p>
          ) : (
            <div className="space-y-6">
                      ...
                      <tbody>
                        {section.capabilities.map((capability) => (
                          <tr key={capability.key} className="border-b last:border-0">
                            ...
                            <td className="px-4 py-3 align-top">
                              <code className="rounded bg-muted px-1.5 py-0.5 text-xs">{capability.key}</code>
                            </td>
                            {roleOrder.map((role) => (
                              <td key={`${capability.key}-${role}`} className="px-4 py-3 text-center align-top">
              ...
              {visibleSections.every((entry) => entry.capabilities.length === 0) && (
          ...
          {roleOrder.map((role) => {
            const count =
              matrix?.sections
                ...
                .filter((capability) => capability.roles.includes(role)).length ?? 0;
            return (
              <div key={role} className="rounded-md border bg-card p-3">
                ...
                <p className="text-xs text-muted-foreground">capabilities mapped</p>
              </div>
            );
          })}
        </CardContent>
