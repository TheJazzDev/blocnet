...
| 2026-09-11 | Scales | — | Retired `AppColors.secondary500`: its categorical half became `tagInfo` (#3B82F6, 5.4:1) and `tagAirdrop`, its gradient half became `primary700`. Added `lib/app/tokens/{text,space,radius,icon}.dart` and migrated 204 files, 3,771 token references (`f063de1`). Two codemod bugs caught mid-flight: `size:` is Flutter's `Icon` parameter as well as ours (285 of 1,117 literals were icons, so icons got their own scale), and a `SizedBox` with a child is sizing, not spacing (82 wrongly rewritten, rule corrected). Three pixel-exact layouts broke and were fixed: skeleton card, level status pill, and its slot. `flutter analyze` clean, 113/113. **Not yet verified on a device** — widget tests cover a fraction of the 43 screens, and both failures found were indirect constraints a static scan cannot see. |
