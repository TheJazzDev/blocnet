...
| E | Dead-code cleanup | all | dead-code register in the atlas (Become Hunter and old profile bodies already handled by WS-F) | [session-templates.md#ws-e](session-templates.md#ws-e--dead-code-cleanup) |
...
|---|---|---|---|---|---|---|---|
| F-01 | P1 | mobile | "Space" concept invisible; switcher is an unlabeled logo icon; Hunter profile hides Badges/Quests/Levels | F | done | WS-F branch `worktree-agent-a4eab73085000e67b` | decided 2026-09-11: keep spaces, make explicit |
...
| F-07 | P2 | cross | Vocabulary drift: Gems/Projects, Members/Users, BNP/BNT/MCR | F + G + H | done | mobile: WS-F branch `worktree-agent-a4eab73085000e67b`; backend: WS-H merged | decided: reconcile Gems/Projects/Updates; glossary; MCR removed everywhere (legacy "Mine Credits" row in dev DB, 2 empty accounts, 0 tx; `*Mcr` fields in mining-admin/wallet-admin services + console types) |
| F-08 | P2 | mobile | Cold start ~7 s with no skeletons | A (stretch) | done | WS-J merged | use `/me/home-bootstrap` cache |
...
| F-10 | P2 | mobile | Dead-end "coming soon" entries (Help & Support ×5, Moderation ×2) | F | done | WS-F branch `worktree-agent-a4eab73085000e67b` | decided: hide until real; ship static FAQ + Getting Started |
...
| F-14 | P2 | mobile | Body text small (49× fontSize 9–10); user comment "kinda tiny" | D | done | `f063de1` | Type scale applied across 824 sites. Prose at 12–13px promoted to body 15px, 8–10px lifted to caption 11px. Split by font weight: w400 is prose, w500+ is a label. |
...
| F-16 | P2 | mobile | Levels page shows no requirements/progress | F | done | design-system commit 3d0dd1b | tiers, requirement rows and progress header added by the levels redesign; WS-F only verifies |
...
| 2026-09-11 | **MCR migration deletes balances, does not convert them.** `20260911130140_retire_mcr_tip_currency` removed the MCR currency plus a user tip account holding 706000 atomic MCR on dev. Before this migration runs against production, confirm no real MCR balances exist or add a BNP conversion step. | F-07 |
| 2026-09-11 | **Production has no MCR balances** (owner confirmed). The retire migration may deploy as-is; no conversion step needed. | F-07 |
...
| 2026-09-11 | **Brand accent — Signal Cyan.** One hue, split by intensity: User `#0891B2`, Hunter `#22D3EE`; Moderation stays `#EF4444` (semantic alarm, not brand). Console adopts the same deep cyan for `--primary`, `--ring`, `--sidebar-primary`, `--sidebar-ring`, replacing the periwinkle `oklch(0.67 0.17 268)`. Reasoning: cyan is the only hue Blocnet already owns that no major social-crypto brand holds, and an intensity split keeps one brand identity across spaces where two hues would halve recall. The old user blue `#2563EB` measured **3.85:1** on `#09090b`, below the 4.5:1 AA floor. Study: [Blocnet Accent Directions](https://claude.ai/code/artifact/9b62ac43-9070-4093-ac8b-6ab32c14b08b). | F-23, console↔mobile alignment |
| 2026-09-11 | **Token-scale sequencing.** Do not normalise type/spacing/radius to today's values, and do not fix them screen-by-screen. Order is: decide the target scale → one value-mapped codemod across ~2,350 literal sites (1,060 `SizedBox`, 638 `EdgeInsets`, 479 radius, 176 `fontSize`) → structural redesign only on the screens that need new hierarchy. The codemod *is* most of the redesign; normalising to current values would cement the 12px body scale. | F-14 |
...
| 2026-09-11 | Coordinator | A+B+C merge | Design system committed (a6727a7). Rebased WS-A and WS-C on top: 3 conflicts resolved (tip_history_screen imports; users/[id] PageClient keeps LevelSection + RolesBadgesTab; levels gating moved into use-levels-page hook). Merged into stage: 7de2e7e (B), 5e8e85f (A), fa76ef4 (C). Mobile 54/54, console 39/39 + build green. Next: live verification, then WS-F/G/H. |
| 2026-09-11 | Coordinator | verify | Live verification on emulator + Chrome after merge (stage @ c89aafc): confirmed F-02 (System Alerts hidden), F-03 (0/0 totals), F-06 (QR receive, swap state, pre-launch line), F-09 (Hunter), F-19 (Edge CTA, hub neutral), F-21 (snackbar), F-22 (subtitle), F-27 (skeletons); console F-04 (no sideways scroll with row menu open), F-05 (@claude_audit, 4 tabs), F-11 (collapsible groups, one-row footer), F-13 (88 view events hidden), F-18 (Show more clamp), F-26 (STAGE badge in header), F-28 (KYC empty state). New: F-29 hydration mismatch. Backend on :3080 restarted by coordinator; Flutter run restarted with new APK. |
| 2026-09-11 | Design system + accent | — | Console UI kit synced to Claude Design ([project](https://claude.ai/design/p/ac45a2a1-0d3c-443e-a3d0-6a5a5e22b701)), 19 components / 76 bundle exports, sync inputs in `.design-sync/` (a6727a7). Accent decision applied to `mobile/lib/app/theme.dart` and `console/app/globals.css`; `flutter analyze` clean. Artifacts snapshotted to `docs/artifacts/` and indexed in `docs/ARTIFACTS.md`. Open: `AppColors.secondary500` (#2563EB) still used in 10 places outside the accent system; `--chart-1` deliberately left periwinkle (categorical, not brand); design system needs a re-sync to pick up the new brand. |
| 2026-09-11 | WS-H agent | H | 4 commits on branch `worktree-agent-a549e46b3bd7360d6`, rebased and merged into stage: MCR retire migration (guarded, no-op when absent), admin guard rejecting MCR, `lifetime*Bnp` fields with deprecated `*Mcr` aliases on `GET /admin/mining/metrics` and wallet health, tips module BNP/BNT doc. Build green, 12 new tests, 5 pre-existing failures unchanged. |
| 2026-09-11 | WS-F agent | F | 4 commits fast-forwarded into stage: F-01 (space chip, explainer sheet, single role-aware ProfileBody), F-20 mobile half (Become Hunter → POST /admin-applications, invites accept/decline in Hunter Hub), F-10 (FAQ, Getting Started, coming-soon entries removed), F-07 mobile (Gems copy, Glossary at /help-support/glossary), F-16 verified. 113/113 tests. Backend gap noted: no `GET /admin-applications/mine`, so rejected applications stay "pending" locally. |
| 2026-09-11 | Coordinator | verify F | Emulator after WS-F build: explainer sheet fires on launch, app-bar chip shows current space and opens switcher, single profile body shows hunter stats + all More entries in User space, Help lists FAQ / Getting Started / Glossary with Live Chat, Bug Report, Account & Privacy gone, Moderation Hub has only Reports Queue and Appeals. |
| 2026-09-11 | WS-G agent | G | 4 commits rebased + merged into stage: F-29 root cause is a React bug in Next 16.1.x (replayed lazy segment drops the Forked flag so useId is 2 bits short); fixed by pinning `next` 16.2.12. F-20 console half: Manage hunters sheet on /projects (assign/invite/list invites), "Owner review required" note on role applications. F-07: MCR stripped from tip settings, mining totals via one adapter reading `*Bnp ?? *Mcr`, "BNP/hour". 47/47 tests, build green. Backend limits assign/invite to platform owner or the project's ownerAdminId, so admins may see 403 inline. |
| 2026-09-11 | Coordinator | verify G + cleanup | Chrome after WS-G merge (Next 16.2.12): no hydration error on /projects, /applications; Manage hunters sheet opens with hunter search, Assign/Invite, and the owner-only invite hint; /tip-settings lists only BNP and BNT; header reads "Users see projects as Gems in the app." All 8 agent worktrees removed and merged branches deleted. **stage is clear for screen/design work.** Remaining: F-08 (stretch), F-14/F-23 (design system), WS-E dead-code cleanup, MCR balance decision before prod migration. |
| 2026-09-11 | Scales | — | Retired `AppColors.secondary500`: its categorical half became `tagInfo` (#3B82F6, 5.4:1) and `tagAirdrop`, its gradient half became `primary700`. Added `lib/app/tokens/{text,space,radius,icon}.dart` and migrated 204 files, 3,771 token references (`f063de1`). Two codemod bugs caught mid-flight: `size:` is Flutter's `Icon` parameter as well as ours (285 of 1,117 literals were icons, so icons got their own scale), and a `SizedBox` with a child is sizing, not spacing (82 wrongly rewritten, rule corrected). Three pixel-exact layouts broke and were fixed: skeleton card, level status pill, and its slot. `flutter analyze` clean, 113/113. Verified on the Pixel 9 Pro emulator (API 36) against the local backend: Home, Discover, Community, Mining, Wallet, Profile, the space-switcher sheet and the public-profile sheet all render with **zero `RenderFlex` overflow errors**. Sampled the rendered accent straight off a screenshot: `#0891B2`, exactly the User-space deep cyan. Remaining screens are unwalked. |
| 2026-09-11 | WS-I agent | I | `GET /admin-applications/mine?targetRole=` merged (AuthGuard only, newest first, 401/400 covered by 13 specs). No `reviewNote` column exists, so the response has none. Live on :3080. Mobile half in WS-J. |
| 2026-09-11 | WS-E agent | E | Dead-code cleanup merged: 19 mobile files (orphan stores, dead routes /mining/downline and /community-staff-tools, submit_appeal, commented connectivity/offline banner, misc zero-import widgets) and 24 console files (unused shared components, v2 hook, 17 migration docs, dead watermark import) removed, −10,355 lines. Alias routes /home /discover /mining kept (used by deep links). Backend untouched; still-unconsumed routes: user-side KYC submit/status, user wallet health, admin social overview, levels leaderboard, delete device token. Mobile 113/113, console 47/47 + build green. |
| 2026-09-11 | WS-J agent | J | Merged: F-08 Home paints from a user-scoped bootstrap cache on first frame (raw feed JSON persisted, stale-while-revalidate 3 d), change-only notifies, skeletons where no cache, in-flight GET coalescing in ApiClient, edge/radar dedupe. Emulator: feed-ready minus first-frame 9.6 s → 4.1 s cold / 0.11 s cached; startup calls 15 → 10. F-30 mobile: application state derived from `/admin-applications/mine`, rejected card allows re-apply, 404 fallback. 120/120 tests. Follow-up: `/me` still fetched twice on boot (AuthStore). |
| 2026-09-11 | Coordinator | verify J | Emulator cold start after merge: first frame paints radar + edge card + feed from cache, no "Loading alpha radar" text; log shows `edge_brief_source=cache`, 10 calls in the first 10 s (was 15). Become a Hunter row correctly absent for an account that already holds the role. |
