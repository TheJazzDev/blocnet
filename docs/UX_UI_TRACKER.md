...
| F-30 | P3 | mobile+backend | No "my application" read endpoint, so a rejected hunter application stays "pending" on the phone | I (+J) | open | | add `GET /admin-applications/mine`; mobile switches from the local prefs flag to the server state |
