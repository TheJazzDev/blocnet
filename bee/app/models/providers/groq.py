...
This provider uses Groq's cloud API with free tier (14,400 requests/day per model).
Advantages: Very fast (500+ tokens/sec), generous free tier, multiple models.
Requirements: Groq API key from https://console.groq.com
"""
...
class GroqProvider(LLMProvider):
    """Free cloud provider with lightning-fast inference."""
    ...
    async def analyze_content(
        ...
        """Analyze crypto update content using Groq."""
...
Be strict but fair. Return only valid JSON."""
    ...
    async def generate_embedding(
        ...
        Note: Groq doesn't provide embeddings API, so we fall back to
    ...
    async def is_available(self) -> bool:
        ...
        self.last_availability_error = None if available else "Groq API key not configured"
        return available

    @property
    def supports_web_search(self) -> bool:
        """Groq does not support web search."""
        return False
