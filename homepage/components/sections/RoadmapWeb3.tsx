...
export function RoadmapWeb3() {
  ...
  useGSAP(() => {
    ...
    const activeIcon = document.querySelector('.active-phase-icon');
    if (activeIcon) {
      gsap.to(activeIcon, {
        scale: 1.15,
        duration: 1,
  ...
  return (
    <section
      ref={containerRef}
      className="py-16 sm:py-20 md:py-24 px-4 sm:px-6 lg:px-8 bg-[#09090b] relative overflow-hidden"
    >
      {/* Background */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        {/* Header */}
        <div className="roadmap-header text-center mb-12 sm:mb-16">
        ...
        {/* Roadmap Grid */}
        <div className="roadmap-grid grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 sm:gap-6 md:gap-8">
          {phases.map((phase, index) => (
            ...
            >
              ...
              {index < phases.length - 1 && (
                <div className="hidden xl:block absolute left-full top-1/3 w-8 h-0.5 bg-linear-to-r from-teal-500/50 to-transparent -translate-y-1/2 z-0" />
              )}
              ...
              >
                ...
                {/* Icon */}
                <div className={`flex justify-center mb-4 sm:mb-5 ${phase.status === 'active' ? 'active-phase-icon' : ''}`}>
                  <div className="w-20 h-20 sm:w-24 sm:h-24 bg-linear-to-br from-teal-500/20 to-primary/20 rounded-2xl flex items-center justify-center border border-teal-500/30">
                    <span className="text-4xl sm:text-5xl">{phase.icon}</span>
                  </div>
                </div>
