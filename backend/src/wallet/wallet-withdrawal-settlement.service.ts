...
export class WalletWithdrawalSettlementService {
  private readonly logger = new Logger(WalletWithdrawalSettlementService.name);
  ...
  async processBroadcastingWithdrawals(
    getClient: (network: WalletDepositNetworkConfig) => PublicClient,
  ): Promise<void> {
    const withdrawals = await this.prisma.withdrawalRequest.findMany({
      where: {
    ...
    for (const withdrawal of withdrawals) {
      await this.processBroadcastingWithdrawal(withdrawal.id, getClient);
    }
  }
