...
/// Builds the text input; wire [onChanged] to it so suggestions follow typing.
typedef UsernameInputBuilder = Widget Function(
  BuildContext context,
  ValueChanged<String> onChanged,
);

/// An @username input with member suggestions from `/profiles/search` as the
/// user types. Picking one fills [controller] with `@username`.
///
/// The input itself comes from [inputBuilder], so each form keeps its own
/// field style; the lookup, debounce and suggestion list live here.
class UsernameSuggestField extends StatefulWidget {
  const UsernameSuggestField({
    super.key,
  ...
  final UsernameInputBuilder inputBuilder;

  /// Injectable for tests; defaults to [UsersApiRepository.searchProfiles].
  final ProfileSearch? search;

  /// Hidden from suggestions (usually the signed-in user).
  final String? excludeUsername;

  @override
  State<UsernameSuggestField> createState() => _UsernameSuggestFieldState();
}

class _UsernameSuggestFieldState extends State<UsernameSuggestField> {
  ...
  Future<void> _lookup(String query) async {
    ...
    try {
      ...
      final excluded = widget.excludeUsername?.toLowerCase();
      setState(() {
        _results = rows
            .where((row) => (row.username ?? '').isNotEmpty)
            .where((row) => row.username!.toLowerCase() != excluded)
            .take(5)
            .toList();
      });
    } catch (_) {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        widget.inputBuilder(context, _onChanged),
  ...
  Widget _suggestion(ProfileSearchResult row) {
    return InkWell(
      key: ValueKey('username-suggestion-${row.username}'),
      onTap: () => _pick(row),
      child: Padding(
