...
part 'projects_store_discovery.part.dart';

class ProjectsStore extends ChangeNotifier with _ProjectsStoreDiscoveryMixin {
  ProjectsStore({
  ...
  Future<void> hydrateFollowStateFromMeSummary(
    Map<String, dynamic>? meSummary, {
    bool notify = true,
  }) async {
    if (meSummary == null) return;

    final followedIds =
        (meSummary['followedProjectIds'] as List<dynamic>? ?? const [])
            .map((value) => value.toString())
            .toSet();
    final parsedPreferences =
        _usersRepository.parseFollowPreferencesFromMe(meSummary);
