...
class CommentsStore extends ChangeNotifier {
  ...
  final Map<String, Timer> _channelRecoveryTimers = {};
  String? _lastError;
  ...
  void unwatchCommentsRealtime(String updateId) {
    _refetchDebounceTimers.remove(updateId)?.cancel();
    _channelRecoveryTimers.remove(updateId)?.cancel();
  ...
  void _scheduleFallbackRefresh(String updateId) {
    _refetchDebounceTimers.remove(updateId)?.cancel();
    _refetchDebounceTimers[updateId] = Timer(_refetchDebounce, () {
      _refetchDebounceTimers.remove(updateId);
      unawaited(refreshLatestWindow(updateId));
    });
  }
  ...
  void _handleSubscribeStatus(String updateId, RealtimeSubscribeStatus status) {
    ...
    if (status == RealtimeSubscribeStatus.channelError ||
      ...
      _scheduleChannelRecovery(updateId);
    }
  }

  void _scheduleChannelRecovery(String updateId) {
    if (_channelRecoveryTimers.containsKey(updateId)) return;
    _channelRecoveryTimers[updateId] = Timer(_channelRecoveryDelay, () {
      _channelRecoveryTimers.remove(updateId);
      final existing = _commentChannelsByUpdateId.remove(updateId);
  ...
  @override
  void dispose() {
    for (final timer in _refetchDebounceTimers.values) {
      timer.cancel();
    }
    _refetchDebounceTimers.clear();

    for (final timer in _channelRecoveryTimers.values) {
      timer.cancel();
    }
    _channelRecoveryTimers.clear();
